...
class BufferManager
{
    ...
    Page insertIntoPool(string tableName, int pageIndex, bool isMatrix = false);
    // Page insertIntoPoolMatix(string tableName, int pageIndex);

public:
    BufferManager();
    Page getPage(string tableName, int pageIndex, bool isMatrix = false);
    void writePage(string pageName, vector<vector<int>> rows);
    void updatePage(Page p);
    void writePage(string tableName, int pageIndex, vector<int> row);
