...
void BufferManager::updatePage(Page p)
{
    for (auto it = this->pages.begin(); it != this->pages.end(); ++it)
    {
        if (it->pageName == p.pageName)
        {
            this->pages.erase(it);
...
void BufferManager::removePage(Page p)
{
    for (auto it = this->pages.begin(); it != this->pages.end(); ++it)
    {
        if (it->pageName == p.pageName)
...
void BufferManager::writePage(string tableName, int pageIndex, vector<int> row)
{
    logger.log("BufferManager::writeRow");
    Page page(tableName, pageIndex, row);
    page.writePage(row);
}
