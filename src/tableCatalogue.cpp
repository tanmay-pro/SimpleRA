...
void TableCatalogue::renameTable(string fromTableName, string toTableName)
{
    logger.log("TableCatalogue::renameTable"); 
    Table* table = this->getTable(fromTableName);
    table->tableName = toTableName;
    this->tables[toTableName] = table;
    this->tables.erase(fromTableName);
}
