...
class Page{
    ...
    string pageName = "";
    Page();
    Page(string tableName, int pageIndex, vector<int> row);
    Page(string tableName,int pageIndex, bool isMatrix = false);
    Page(string tableName, int pageIndex, vector<vector<int>> rows, int rowCount);
    vector<int> getRow(int rowIndex);
    void writePage();
    void writePage(vector<vector<int>> rows);
    void writePage(vector<int> row);
